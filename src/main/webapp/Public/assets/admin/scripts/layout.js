    ...
    var loginInfo = function () {
    	if(isLoginUrl()){
	    	$.ajax({
				...
				dataType : "json",
				success : function(res) {
					$('#userName').html("<i class='icon-user' style='width:1.5em;font-size:0.9em;color: rgb(171, 163, 14)'></i>"+res.userName);
				}
	    	});
    	}
    };
...
function doOnLoadMenu(){
	...
	if(parentMenu&&childMenu){
		...
		$("#selem-1").removeClass("selected");
		$("#selem-1").addClass("arrow");
	}else{
//		$("#selem-0-0").addClass("active");
		$("#selem-1").removeClass("arrow");
		$("#selem-1").addClass("selected");
	}
	
	if(parentMenu){
		$("#menu-"+parentMenu).addClass("active");
		$("#selem-"+parentMenu).removeClass("arrow");
		$("#selem-"+parentMenu).addClass("selected");
	}
